...
//
// Sub-stage 1.5.3: anti-AI rules added for consistency across all agents.
...
3. **כתוב בלשון "אנו" או בלשון סתמית, לא בלשון אישית.** אתה מדווח על המערכת, לא מנהל שיחה.
...
🚫 חוקי כתיבה — חובה לציית בכל הדוח:

1. אסור em-dash (—) או en-dash (–) בשום מקום בדוח. במקומם נקודה, פסיק, או מקף רגיל (-).
2. אסור hashtags (#) בכלל.
3. אסור emojis בדוח אנליטי.
4. ביטויים אסורים (נשמעים AI):
   - "תודה על פנייתך"
   - "אנחנו כאן בשבילך"
   - "צוות מקצועי"
   - "פתרון מקיף"
5. כתוב ענייני, ישיר, ישראלי. לא flowery.
