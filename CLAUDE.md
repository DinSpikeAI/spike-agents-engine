...
### 10.44 Sprint 3I Onboarding Integration — Day-1 Brief for New Tenants (DONE, commit `87e2b20`) — 2026-05-15

Sprint 3I (Phase 2) shipped on 2026-05-13 with the `business_brief` field injected into 5 customer-facing Sonnet agents via the `/dashboard/settings` page. The gap that surfaced: a NEW tenant signing up after 3I ships starts with an EMPTY brief because onboarding never asked for it. The five customer-facing agents fall back to no-brief behavior until the owner finds and fills the settings page. For a design partner who completes onboarding and immediately tests "create a draft" → they see generic AI output, not their voice. The Sprint-3I "killer-differentiator" never fires on Day 1.

**Fix (3 files, single commit):**
- **MODIFIED** `src/app/onboarding/actions.ts` — added optional `businessBrief?: string` to `OnboardingFormData`, server-side validation (max 2000 chars matching settings page), and a single write to `newConfig.business_brief` ONLY when the input is non-empty. Empty input preserves any prior value in `existingConfig` (rare edge case where a user navigated to `/dashboard/settings` before completing onboarding) rather than clobbering with empty string. Same `MAX_BUSINESS_BRIEF_LENGTH = 2000` constant pattern as the settings page.
- **MODIFIED** `src/app/onboarding/onboarding-form.tsx` — added a 5th field at the bottom: textarea with label "מה מאפיין את העסק שלך? (אופציונלי)", a char counter "0/2000", Hebrew helper text ("הסוכנים ינסחו ללקוחות שלך בסגנון שלך — תיאור קצר עוזר להם להישמע כמוך מהיום הראשון"), and a placeholder demonstrating the Sprint-3I demo pattern ("לדוגמה: מספרה קטנה בעין השופט. אני מתמחה בקרטין. אוהבת לקוחות 'יקירה'..."). `businessBrief` is NOT in the `canSubmit` gate — owner can skip and fill via settings later. The textarea includes `resize: vertical` for users with verbose briefs.
- **MODIFIED** `src/app/onboarding/page.tsx` — cosmetic copy update: "ארבעה פרטים בלבד לפני שנתחיל" → "כמה פרטים לפני שנתחיל" (since the 5th field is optional, the original "only four details" claim was now misleading).

**Effect on day-one production:** any tenant who completes onboarding from this commit forward and types into the brief textarea gets all 5 customer-facing Sonnet agents producing voice-matched drafts from their very first run. Day 1 of the design partner experience matches Day 30 — no "wait, my drafts sound generic" gap. Owners who skip the textarea still get the existing fallback behavior (agents produce competent Hebrew without the brief block) and can fill brief via `/dashboard/settings` whenever they discover the gap.

**Verification:** build green; `/api/onboarding` route registered; SQL-tested by temporarily resetting `config.onboarding_completed_at = NULL` on the demo tenant to revisit `/onboarding` — confirmed the 5th field renders correctly with the counter and helper text. ⚠️ **Edge case observed during verification:** for a tenant that already has `onboarding_completed_at` set, a stale JWT cookie can keep redirecting to `/dashboard` even after the DB column is reset. This is JWT caching, not a code bug; logout + login refreshes the cookie. Production users hitting `/onboarding` for the first time (with no prior session) don't encounter this.

---

### 10.45 Sonner Toaster Migration — alert() → toast.error (DONE, commit `b49bcb9`) — 2026-05-15

`<Toaster />` from `@/components/ui/sonner` was already mounted at the root layout (`src/app/layout.tsx`) with `richColors position="top-center" dir="rtl"`. But 5 places in the dashboard code still used the browser-native `alert()` for error feedback, which is jarring (modal-blocking), un-styled, and ignores the RTL preference. §19.8 had this listed as "Sonner Toaster migration" without specifying scope.

**Scope discovered via `Get-ChildItem -Path src -Recurse -Filter *.tsx | Select-String -Pattern 'alert\('`:** exactly 5 alert calls, all in two files:
- `src/components/dashboard/approvals-list.tsx`: line 106 (clipboard copy failure), line 132 (approve action failure), line 143 (reject action failure)
- `src/components/dashboard/leads-board.tsx`: line 373 (markLeadContacted failure), line 384 (dismissLead failure)

**Fix:** added `import { toast } from "sonner";` to both files, replaced each `alert(...)` with `toast.error(...)`. Zero logic changes — exact same trigger conditions, exact same Hebrew error strings, just routed through Sonner instead of the browser. 2 files, 5 replacements + 2 imports = 7 lines added, 5 lines removed (net +2). Build green; post-deploy verification confirmed 0 `alert()` calls remaining in `src/**.tsx`.

**Why this matters:** error UX in the approvals flow is the highest-friction failure surface in the product — when an owner taps "אשר ושלח" and something fails, getting a styled toast that doesn't block the page (vs a modal dialog that blocks everything until dismissed) is the difference between "the system feels broken" and "the system reported something I should fix". Sonner toasts auto-dismiss, are dismissible with X, and respect RTL — they look like a polished product, not an HTML form from 2005.

**Scope explicitly excluded:** success toasts on approve/reject. The current behavior on success (`router.refresh()`) makes the draft card disappear, which is its own visual confirmation. Adding `toast.success("הטיוטה אושרה")` would be redundant feedback. If user research surfaces that owners miss the confirmation, easy to add later — but for now, less is more.
...
- **Vault encryption for `integrations.metadata.access_token`.** Currently plaintext JSONB; pre-launch debt per §11.2. Single migration + small `lookupWhatsAppIntegration` (drafts.ts + growth.ts) + admin/integrations actions update. **Premature pre-עוסק-מורשה** — currently only 24h temp Meta tokens (1 demo tenant, no real customers, token rotates daily anyway), so plaintext exposure window is minimal. Worth doing when (a) the Meta system-user token arrives and stops auto-expiring, AND (b) real customer data is in flight. Until then, defer.
- ~~**Sonner Toaster migration.**~~ ✅ DONE 2026-05-15 — see §10.45 + Sprint 3W moved this from "hygiene backlog" to "shipped". 5 `alert()` calls replaced with `toast.error()` across `approvals-list.tsx` (3) and `leads-board.tsx` (2). `<Toaster />` was already mounted in root layout. Commit `b49bcb9`.
