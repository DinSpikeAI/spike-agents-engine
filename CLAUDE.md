...
>
> **Last updated:** 2026-05-15 evening (post-session: **Owner-facing trinity COMPLETE** — 3M + 3X + 3Y all shipped + Sprint 3W internal hygiene). **What this means in plain terms:** the business owner now receives three operational signals on WhatsApp automatically, with zero dashboard interaction required: a Hebrew daily summary at 07:00 IL (Sprint 3M, shipped 2026-05-10), real-time critical/high alerts as Watcher classifies them (Sprint 3X, shipped today), and a weekly health digest every Sunday at 08:00 IL (Sprint 3Y, shipped today). All three use the same Iron-Rule carve-out per §15.25 (owner-self loopback exempt from the `[אשר]` flow because the recipient is the owner of the producing tenant). **What shipped today:** (1) Sprint 3X `e3bff86` — `src/app/api/cron/watcher/route.ts` refactored from sequential loop to per-tenant `processTenant` with chunked `Promise.allSettled(5)`; added `maybeSendCriticalAlerts` with diff-based dedupe (signature `${category}|${source}|${occurredAt}` compared against previous successful Watcher run's alerts — no new DB column) + `renderWatcherAlertsMessage` (Hebrew body with severity labels from `SEVERITY_LABELS_HE`); severity threshold set at `severityRank<=high` covering the 6 categories: negative_review/customer_complaint/urgent_message/new_lead (critical) + payment_issue/hot_inquiry (high). (2) Sprint 3Y `3decc46` — NEW route `src/app/api/cron/manager/route.ts` (folder created from scratch); modified `vercel.json` adding 9th cron `{ "path": "/api/cron/manager", "schedule": "0 5 * * 0" }`; weekly idempotency via `weekStart = Sunday 00:00 UTC of current calendar week`; render output as compact teaser (5-10 lines, empty sections omitted) linking to `/dashboard/reports`. (3) Sprint 3W `451beae` — internal hygiene: removed always-failing `agent_runs` insert with `tenant_id: null` from `src/app/api/cron/cleanup/route.ts` (cleanup is platform-scope, has no meaningful tenant_id; results live in JSON response + console.log); replaced Next.js scaffold `README.md` with a proper engine README; 4 comment-only fixes in `src/app/api/cron/watcher/route.ts` ("hourly" → "daily" matching `vercel.json` `0 6 * * *`). **End-to-end WhatsApp delivery deferred for 3X + 3Y** on Meta access-token refresh (the multi-day session pause left the temporary 24h token expired); SQL inspection of historical Watcher runs confirmed 3X dedupe + filter logic would have triggered correctly (1 critical/high alert on 05-14 06:18 → would send 1; 2 critical/high on 05-13 15:43 → would send 2). The send wiring is the same `lookupWhatsAppIntegration` + `wasContactedInLast24h` + `mapSendErrorToHebrew` + `sendWhatsAppMessage` helpers proven by Sprint 3M's 3 real-world deliveries — first 3X + 3Y deliveries will happen automatically once the Meta token is refreshed. **Earlier in this period (Sprint 3I, 2026-05-13):** Phase 1 unblock `c4b6942` (§15.33 type extraction pattern) + Phase 2 Batches 1+2+3 (`f39b26b` + `806904a` + `d1eba6a` — settings UI + 4 Sonnet agents + Growth) shipped the **killer-differentiator**: every customer-facing draft matches the owner's voice on first generation. 5 customer-facing agents inject `business_brief` from `tenants.config`; Batch 4 (Morning + Watcher + Manager + Inventory + Hot Leads) descoped because those agents are owner-facing or produce classification JSON only. **Validated end-to-end on demo tenant** with brief = *"סלון יופי קטן בעין השופט. אני מתמחה בקרטין. אוהבת לקרוא ללקוחות 'יקירה'"*: Growth produced a reactivation draft for דנה כהן with *"דנה יקירה"* + *"חידוש הקרטין"* + *"הקרטין מחזיק בערך עד כאן בדיוק"* + *"סליחה על זה"* — three brief-injection signals in one draft, all natural Hebrew owner-voice. **Earlier same day 2026-05-13:** dashboard runtime fix `7539dcd` (edge → nodejs, gave heavy Sonnet agents 60s budget instead of 25s). **4 new lessons / documents added today:** §15.33 (type extraction pattern), §10.40 (3I Phase 2), §10.41 (3X), §10.42 (3Y), §10.43 (3W). **The product is functionally complete for design partner #1.** External blockers only: עוסק מורשה / Meta Business verification / business phone number (paperwork, not code). **Strategic decisions locked — see §19** (pricing is OPEN QUESTION — §19.1 thesis "single package, all agents" stands but exact number TBD; §12.1's old Solo/Pro/Chain table is deprecated, see header note in §19.1): BSP 360dialog primary, Meta Cloud direct fallback; wedges = [אשר] button (TM-pending) → voice notes → no-shows ROI; channel = periphery cities + bookkeepers + Achiya rev-share. **Latest commits (newest first):** `451beae` (3W hygiene), `3decc46` (3Y Manager weekly), `e3bff86` (3X Watcher auto-send), `af59fe2` (Batch 5b CLAUDE.md backfill), `d1eba6a` (3I Batch 3 Growth), `d5ca7ad` (Batch 5 docs sweep), `806904a` (3I Batch 2), `f39b26b` (3I Batch 1), `c4b6942` (§15.29 fix).
...
### 6.1 The 9 Customer-Facing Agents

| # | Agent | Model | Trigger | Output | withRetry? | Anti-AI? | brief? |
|---|---|---|---|---|---|---|---|
| 1 | Manager | sonnet-4-6 | Weekly cron (Sun) | `manager_reports` | ✅ | ✅ | — (owner-facing) |
| 2 | Morning | haiku-4-5 | Daily cron 07:00 IL (3M) | `agent_runs.output` JSONB + **WhatsApp auto-send to owner** (3M) | ✅ | ✅ | — (owner-facing, §15.25) |
| 3 | Watcher | haiku-4-5 | Real-time webhook + daily cron | `alerts` | ✅ | ✅ | — (owner-facing alerts) |
| 4 | Reviews | sonnet-4-6 | New review event | drafts (review_reply, platform=google_business_profile → copy-paste) | ✅ | ✅ + Israeli-tone | ✅ (Sprint 3I Phase 2 Batch 2) |
| 5 | Hot Leads | haiku-4-5 | Real-time webhook | `hot_leads` + cascade to Sales QR | ✅ | ✅ | — (classification JSON only, no customer text) |
| 6 | Social | sonnet-4-6 | Cron 05:30 (no Sat) | drafts (social_post, platform=manual_paste → copy-paste) | ✅ | ✅ + hashtags removed | ✅ (Sprint 3I Phase 2 Batch 2) |
| 7 | Sales | sonnet-4-6 + thinking | TWO entry points §6.8 | drafts (sales_followup × email/IG → copy-paste; sales_quick_response → WhatsApp send) | ✅ | ✅ | ✅ both entry points (Sprint 3I Phase 2 Batch 2) |
| 8 | Inventory | sonnet-4-6 | Cron 05:30 Sun/Wed | `agent_runs.output` JSONB + `inventory_snapshots.last_analyzed_at` (NOT drafts — see §10.39 for the validation pass that confirmed this) | ✅ | ✅ | — (owner-facing inventory snapshots) |
| 9 | Growth | haiku-scan + sonnet-draft | Weekly cron (Sun) + on-demand | `growth_candidates` + drafts (growth_reactivation → WhatsApp send) | ✅ | ✅ | ✅ (Sprint 3I Phase 2 Batch 3 — Sonnet draft path; 3rd NON-cached block after the 2 existing cached blocks) |

**As of 1.5.3:** ALL 9 agents have anti-AI hygiene at both prompt level AND post-processing level.

**As of Sprint 3I Phase 2 Batch 3 (2026-05-13):** the `business_brief` column reflects which agents inject the owner-authored voice brief into their system prompts. **5 customer-facing agents** are eligible (Reviews, Sales×2, Social, Growth) — those that produce customer-facing text. **All 5 DONE.** Morning/Hot Leads/Manager/Watcher/Inventory are owner-facing or produce non-text output, so brief is irrelevant (Batch 4 descope rationale; see §10.40).
...
### 10.40 Sprint 3I Phase 2 — Business Brief Injection into Customer-Facing Agents (COMPLETE — all 3 batches shipped; Batch 4 DESCOPED) — 2026-05-13

After §15.29 was resolved (commit `c4b6942`, type extraction pattern — see §15.33), Sprint 3I moved from "BLOCKED" to actively shipping. The phase was split into 4 batches; 2 are done, 1 is pending, 1 was descoped after analysis.

**Goal:** persist an owner-authored Hebrew description of their business voice (free-form text up to 2000 chars), and inject it into every customer-facing agent's system prompt so drafts already match the owner's voice on first generation. The killer-differentiator §19.7 called out as the missing piece — Spike's drafts had been "generic Hebrew SMB voice" rather than "this owner's voice".

**Batch 1 — Settings UI (DONE, commit `f39b26b`).** Three things shipped together:
- `src/app/dashboard/settings/types.ts` — added `BUSINESS_BRIEF_MAX_LENGTH = 2000`, `businessBrief: string | null` field in `TenantSettingsInput`.
- `src/app/dashboard/settings/actions.ts` — `updateTenantSettings` now validates brief length (≤2000 chars), normalizes empty/whitespace to `null` before persist, writes to `tenants.config.business_brief` (top-level JSONB key, not nested under any agent sub-config — it describes the business as a whole).
- `src/components/dashboard/settings-form.tsx` — new Card 3 "סגנון העסק שלך" with a 7-row textarea, character counter that turns amber at 90% and pink at the cap, importing `BUSINESS_BRIEF_MAX_LENGTH` from `./types`.
- `src/app/dashboard/settings/page.tsx` — loads `business_brief` from config; also fixed a pre-existing plural-gender bug where `as "male" | "female"` cast silently downgraded plural tenants to male in the form (resolved by using the broader `BusinessOwnerGender` type from `./types` and `VALID_GENDERS` runtime check).

**Verification on demo tenant:** Dean typed *"סלון יופי קטן בעין השופט. אני מתמחה בקרטין. אוהבת לקרוא ללקוחות 'יקירה'"*, clicked save, toast "ההגדרות נשמרו" appeared, SQL `SELECT config->>'business_brief' FROM tenants WHERE id='15ef2c6e-…'` returned the value. Plural gender test (UPDATE → form shows כללי button selected → revert UPDATE) also passed.

**Batch 2 — Injection into 4 Sonnet agents via `withGenderLock` (DONE, commit `806904a`).** The brief is added to the Anthropic `system` array AFTER the cache breakpoint, preserving §15.32's prompt caching (the static prompt prefix stays cached; only the dynamic gender + brief blocks vary per tenant).

Files modified:
- **NEW** `src/lib/safety/business-brief.ts` — exports `extractBusinessBrief(config)` (safe JSONB extraction returning `null` for missing/non-string/empty/whitespace) and `buildBusinessBriefBlock(brief)` (formats brief as a `<business_voice>…</business_voice>` XML-style block with defensive tag-injection escape; closing instructions clarify brief affects tone/word-choice/values but does NOT override hard safety rules like em-dash bans, hashtag bans, price-disclosure controls).
- **MODIFIED** `src/lib/safety/gender-lock.ts` — `withGenderLock(staticPrompt, gender, brief?)` accepts an optional 3rd `brief` parameter. When provided, a 3rd text block (the brief, formatted by `buildBusinessBriefBlock`) is appended after the gender block. Backwards-compatible — existing 2-arg callers continue to work unchanged.
- **MODIFIED** `src/lib/agents/reviews/run.ts` — added `businessBrief: string | null` to `TenantSafetyContext`; `loadTenantContext` extracts brief in the same DB query (no extra roundtrip); passed as 3rd arg to `withGenderLock`.
- **MODIFIED** `src/lib/agents/sales/run.ts` — same pattern in the SHARED `loadTenantContext` (one place serving BOTH entry points); brief passed to `withGenderLock` at both call sites (`runSalesAgent` line ~433 + `runSalesQuickResponseOnEvent` line ~703).
- **MODIFIED** `src/lib/agents/social/run.ts` — same pattern; brief lives at TOP-level config, not nested in `config["social"]` sub-object.

**Verification (definitive, demo tenant, 2026-05-13 evening):** vertical set to `יופי / מספרה`, gender = `male`, brief = the salon description above. Social agent ran via "הרץ עכשיו" → 3 posts at `/dashboard/approvals`:
- **Evening post** contained *"...שתפו בתגובות, **יקירים ויקירות**, ואולי נוציא מזה גם כמה טיפים..."* — the brief's "יקירה" address adapted naturally into the content.
- **Afternoon post** contained *"...עכשיו זה הזמן הנכון לטיפול שישמור על השיער... **טיפול קרטין**, מסכות משקמות..."* — the brief's "קרטין" specialty manifested in concrete content.
- All 3 posts salon-themed (hair care, summer damage, restorative treatments), natural Hebrew, no AI tells.

This is §19.7's "drafts already in owner's voice on first generation" thesis confirmed in production.

**Batch 3 — Growth Agent injection (DONE, commit `d1eba6a`).** Growth uses a different prompt-caching pattern than the 4 Sonnet agents covered in Batch 2: instead of a single cached static prompt prefix (the `withGenderLock` shape), Growth's `draft.ts` has TWO cached blocks — the static `SONNET_DRAFT_SYSTEM_PROMPT` (block 0) AND the per-tenant context returned by `buildTenantContextBlock` (block 1). Both have `cache_control: { type: "ephemeral", ttl: "1h" }`. The brief is added as a THIRD block with **no `cache_control`** — it's the dynamic tail. Rationale: business name / vertical / tone_notes (in block 1) change rarely; the brief is the field owners iterate on most. Placing brief outside the cached region means brief edits don't invalidate the cache for the rest of the tenant context, maximizing cache wins over time.

Files modified (3, surgical):
- **MODIFIED** `src/lib/agents/growth/prompts.ts` — added `businessBrief: string | null` field to `TenantContextForGrowth` interface (with a comment explaining why brief is NOT included in `buildTenantContextBlock`).
- **MODIFIED** `src/lib/agents/growth/draft.ts` — imported `buildBusinessBriefBlock` from `@/lib/safety/business-brief`, hoisted the inline `system: [...]` array into a named `systemBlocks` variable, conditionally pushed the brief block (only when `tenantContext.businessBrief` is truthy — keeps prompt minimal for new tenants).
- **MODIFIED** `src/lib/agents/growth/run.ts` — imported `extractBusinessBrief` from `@/lib/safety/business-brief`, added `businessBrief: extractBusinessBrief(config)` to the return of `loadTenantContextForGrowth` (same DB row already being read for name/config — no extra roundtrip).
- **NOT MODIFIED** `src/lib/agents/growth/scan.ts` — Haiku scan classifies candidates only (no customer-facing text generation), so brief is irrelevant there. scan.ts receives the new field via the shared `TenantContextForGrowth` type but doesn't use it. Consistent with the Batch 4 descope logic.

**Verification on demo tenant 2026-05-13 evening:** triggered Growth via "הפעל סוכן ידנית" button on `/dashboard/growth`. Growth scan + draft pipeline produced (among others) a reactivation draft for דנה כהן (score 75, label "שאלה על חידוש קרטין", 65 days since last interaction):
> *"היי דנה יקירה! ראיתי שפנית בנוגע לחידוש הקרטין ולא חזרתי אלייך, סליחה על זה. הקרטין מחזיק בערך עד כאן בדיוק, אז זה הזמן המושלם לתאם 😊 מתי נוח לך?"*

Three brief-injection signals visible in one draft:
1. **"דנה יקירה"** — the brief's "אוהבת לקרוא ללקוחות 'יקירה'" combined with the customer's actual first name. Not generic "יקירה" — personalized with context.
2. **"חידוש הקרטין"** + **"הקרטין מחזיק בערך עד כאן בדיוק"** — the brief's "אני מתמחה בקרטין" surfaced as professional specialty knowledge (the durability statement is owner-voice expertise, not Haiku-classifier output).
3. **"סליחה על זה"** — natural Hebrew apology in business-owner register, not an AI-clichéd "אנו מתנצלים על העיכוב".

This is the §19.7 "killer-differentiator" thesis ("drafts already in owner's voice on first generation") confirmed in the agent that ships the highest-value WhatsApp drafts (Growth Reactivation has the proven 2026-05-08 real-delivery track record).

**Batch 4 — Morning + Hot Leads + Watcher + Manager + Inventory DESCOPED (analysis, no code).** The original §19.7 claim of "7 agents to inject" was wrong. Correct list is **5 customer-facing agents** (Reviews + Sales×2 + Social + Growth — all done). Rationale for the rest:
- **Morning agent:** owner-facing per §15.25 carve-out. Recipient IS the business owner via WhatsApp auto-send at 07:00 IL. No customer voice to imitate. Brief is irrelevant — the agent speaks TO the owner, not FOR the owner.
- **Hot Leads agent:** produces classification JSON only (`bucket` + `reason` + `suggested_action`). No customer-facing text. The customer-facing text in the Hot Leads flow comes from the Sales QR cascade, which IS customer-facing and DOES inject brief (covered by Batch 2). Adding brief to Hot Leads would be noise that affects nothing downstream.
- **Manager / Watcher / Inventory:** all owner-facing (manager_reports, alerts, inventory_snapshots). Not customer text. Brief irrelevant for the same reason as Morning.

This descope saves ~2 hours of work that would have produced no observable benefit and avoids polluting non-customer-facing agents with content unrelated to their function.

**Commits:** `c4b6942` (§15.29 type extraction fix, Phase 1 unblock) + `f39b26b` (Batch 1, settings UI) + `806904a` (Batch 2, 4 Sonnet agents) + `d1eba6a` (Batch 3, Growth).

**Architectural note (§15.32 caching preserved):** the brief block's position AFTER the cache breakpoint matters. If it had been added BEFORE the breakpoint, every tenant-specific brief would invalidate the static-prompt cache for that tenant, costing ~25% more on first runs and eliminating the ~80% input-token discount on subsequent runs within the 1h TTL. By placing it after, the cached static prompt stays cached across all tenants while each tenant's brief is a tiny dynamic suffix. Verified at runtime: `agent_runs.usage.cache_read_input_tokens` > 0 on second runs within the TTL window post-Batch-2 deploy.

---

### 10.41 Sprint 3X — Watcher Auto-Send to Owner (DONE, commit `e3bff86`) — 2026-05-15

The second Iron-Rule carve-out per §15.25 — Watcher alerts to the business owner. After Sprint 3M proved owner-self loopback works end-to-end (Morning daily summary auto-delivered 2026-05-10), 3X extends the same pattern to Watcher's critical/high-severity alerts. The owner now gets a WhatsApp ping any time Watcher classifies a new event into the most urgent two severity tiers — `critical` (negative_review, customer_complaint, urgent_message, new_lead) or `high` (payment_issue, hot_inquiry) per the `CATEGORY_SEVERITY` map in `src/lib/agents/watcher/hierarchy.ts`. Medium and low severities remain dashboard-only to avoid notification flood.

**Single file modified:** `src/app/api/cron/watcher/route.ts`. The route was refactored from a sequential `for...of` loop into a `processTenant(db, tenantId)` per-tenant helper invoked via chunked `Promise.allSettled(5)` — mirrors the Morning cron pattern from §10.39. Two new internal functions added: `maybeSendCriticalAlerts(db, tenantId, currentResult)` handles the dedupe + integration check + send, and `renderWatcherAlertsMessage(alerts)` formats the Hebrew WhatsApp body. No new DB columns, no new migrations, no new shared helpers — the existing `lookupWhatsAppIntegration` / `wasContactedInLast24h` / `mapSendErrorToHebrew` / `sendWhatsAppMessage` from §10.39 cover everything.

**Dedupe approach (no new state column):** the previous version's failure mode would have been notification flood — Watcher's daily cron classifies events from the last 24h, so a critical alert that persists across runs (e.g., an unanswered new_lead from yesterday morning still in today's 24h window) would get re-sent every run. Instead of adding a `last_watcher_alert_sent_at` column to `tenants`, we use a **diff-based dedupe against the previous successful Watcher run's output**:

1. Filter current run's `output.alerts` to severities at or above `high` via `severityRank(severity) <= severityRank("high")`.
2. Query `agent_runs` for the two most recent successful Watcher runs for this tenant (LIMIT 2 ORDER BY finished_at DESC). Index 0 is the run we just made (already persisted by runAgent); index 1 is the previous run if it exists.
3. Compute signature `${category}|${source}|${occurredAt}` for each alert.
4. `newAlerts = currentHighSeverity.filter(a => !prevSigs.has(signatureOf(a)))`.
5. If `newAlerts.length > 0` → render Hebrew body → run the same three gates as Morning (owner_phone → integration → 24h window) → send.

Edge cases handled by construction: first run for a tenant (no previous run → all current alerts treated as new); event drops out of 24h window then re-appears (not in previous → treated as new, correct since the situation re-emerged); same event continuing across runs (in both current and previous → diff empty → skipped). The signature collision risk is `(category, source, occurredAt)` matching across two distinct events — possible but rare, and the worst case is one missed ping, not a duplicate flood.

**Severity decision documented:** threshold set at `"high"` rather than just `"critical"` because the `high` tier includes `hot_inquiry` and `payment_issue` — both of which need owner attention within the hour but aren't dramatic enough to be `critical`. Setting the threshold any higher would mean the owner only learns about hot inquiries when they happen to check the dashboard. Setting it lower (`medium`) would surface scheduling and inventory items via WhatsApp, which are better reviewed in batch on the dashboard.

**Outcome reporting:** the cron's response JSON now includes `auto_send.tally` showing per-tenant outcomes (`sent`, `no_critical_or_high`, `no_new_since_previous`, `no_owner_phone`, `no_integration`, `missing_credentials`, `outside_24h`, `send_failed`, `skipped_agent_not_succeeded`, `uncaught_error`). This is essential for debugging — when the owner says "I didn't get a ping today", the tally tells us which gate stopped it.

**Verification on demo tenant (2026-05-15):** SQL inspection of historical Watcher runs confirmed 3X would have triggered correctly: 05-14 06:18 run had 1 alert with 1 critical/high (would send 1); 05-13 15:43 + 07:38 runs had 4 alerts each with 2 critical/high (would send 2 each). End-to-end WhatsApp delivery test deferred because the Meta access token expired during the multi-day session pause; refresh requires either a 24h temp token (worth nothing post-refresh) or a system-user token (requires Meta Business verification — post-עוסק-מורשה). The 3X codepath uses the exact same `sendWhatsAppMessage` + helpers as 3M which has 3 proven real-world deliveries, so the send wiring is not in question. **Sprint 3X is code-complete; first real delivery will happen automatically when the Meta token is refreshed.**

**Schedule note:** the Watcher cron is `0 6 * * *` UTC in `vercel.json` (daily ~09:00 IL summer / 08:00 IL winter), NOT hourly — a comment in the route file previously said "hourly" but vercel.json has always been daily. Comments corrected in Sprint 3W (§10.43). The daily cadence is fine for the auto-send pattern: the dedupe ensures no duplicates, and the webhook handler still triggers Watcher in real time via `waitUntil(runWatcherAgent(...))` for fresh events as they arrive.

---

### 10.42 Sprint 3Y — Manager Weekly Auto-Send to Owner (DONE, commit `3decc46`) — 2026-05-15

The third Iron-Rule carve-out per §15.25 — Manager weekly health digest to the business owner. Completes the owner-facing auto-delivery trinity: Morning daily summary (3M), Watcher real-time critical/high alerts (3X), Manager weekly recap (3Y). Owner now receives Spike's three operational signals on WhatsApp without ever touching the dashboard if they don't want to.

**Two files modified:**
- **NEW** `src/app/api/cron/manager/route.ts` (the folder didn't exist before — created by the deploy PowerShell). The route mirrors the Morning cron structure: auth via CRON_SECRET → tenant discovery via WhatsApp `integrations.status='connected'` → eligibility filter on `tenants.config.owner_phone` → chunked `Promise.allSettled(5)` per-tenant processing.
- **MODIFIED** `vercel.json` — added a 9th cron entry: `{ "path": "/api/cron/manager", "schedule": "0 5 * * 0" }`. Fires Sunday 05:00 UTC = **08:00 IL Sunday morning** — one hour after Morning so the owner has had time to read their daily summary before the weekly digest arrives.

**Per-tenant `processTenant(tenantId, ownerPhone)` flow:**

1. **Weekly idempotency check.** Compute `weekStart = Sunday 00:00 UTC of current calendar week` via `Date.setUTCDate(getUTCDate() - getUTCDay())`. Query `agent_runs` for any successful Manager run for this tenant since `weekStart`. If 1+ exist → return `outcome: "already_ran_this_week"`. This protects against retry storms (Vercel may retry on transient failures) and manual triggers that overlap with the scheduled fire.
2. **Run `runManagerAgent(tenantId, "scheduled", 7)`.** Manager already has its own internal spend cap protection via `assertWithinSpendCap` + `reserve_spend` RPC — if the cap blocks, the agent returns `status: "failed"` with a Hebrew error in `result.error`. The cron does not need additional cost gating.
3. **Render to a compact Hebrew WhatsApp body** via `renderManagerSummary(output)`. See below.
4. **Same three gates as Morning:** owner_phone resolution → `lookupWhatsAppIntegration` → `wasContactedInLast24h`. Outside the 24h Meta session window → outcome `outside_24h` (post-Meta-verification this becomes a non-issue via approved templates).
5. **`sendWhatsAppMessage` + `mapSendErrorToHebrew`** on failure.

**`renderManagerSummary` design — teaser, not full report:** `ManagerAgentOutput` is rich (5 sections + recommendation per `MANAGER_AGENT_OUTPUT_SCHEMA`): `summary`, `status_summary` (per-agent run counts + totals), `quality_findings` (drafts sampled + flagged items with severity), `system_health` (cost anomalies + signals + costWindowIls), `growth_metrics` (approvalRate + medianTimeToApprovalMinutes + stalePendingDraftsCount + staleBlazingLeadsCount), `recommendation` (titleHe + detailHe + suggestedActionHe + type), plus the `hasCriticalIssues` boolean. Dumping the whole thing as a WhatsApp body would be 50+ lines — instead, the cron renders a 5-10 line teaser:

- Header: `🗓 Spike — דוח שבועי`
- Agent's one-line `summary` field (always present)
- Section header `📊 *סיכום שבועי:*` with `✅ הצליחו: X    ❌ נכשלו: Y` (only if either is > 0)
- Section header `📈 *מדדים:*` with approval-rate and median-time-to-approval lines (only if those fields are non-null)
- Stale warnings: `⚠️ N טיוטות ממתינות מעל 24 שעות` and/or `🔥 N לידים בוערים לא נענו 24h+` (only if counts > 0; correct Hebrew singular/plural inflection)
- Critical attention block `🚨 *נדרשת תשומת לב:*` listing up to 2 critical quality findings + up to 2 critical system_health signals (only if `hasCriticalIssues === true`)
- Recommendation `💡 *המלצה:* <titleHe>` + suggested action line (only if `recommendation.type !== "no_action_needed"`)
- CTA footer: `דוח מלא: app.spikeai.co.il/dashboard/reports`

The empty-section omission means a healthy week produces a 4-line message; a week with issues produces a 10-15 line summary. Owner scans in 30 seconds; click-through to the dashboard is the action.

**Cost note:** Manager is the most expensive agent (~₪0.50/run with Sonnet 4.6 + thinking budget of 8000 tokens — see `src/lib/agents/manager/run.ts` line 56). The weekly cadence means 4 runs/month/tenant → ~₪2/month/tenant for the Manager dimension alone. This is the dominant per-tenant Anthropic cost item; sales/social/growth at Haiku 4.5 are much cheaper. The internal spend cap fires before any LLM call if a tenant has used too much in the current month.

**Verification:** push successful, build green (output included `/api/cron/manager` in the route list), `vercel.json` validated to 9 crons via ConvertFrom-Json in the deploy script. End-to-end WhatsApp delivery deferred along with 3X — same Meta token blocker. Manager itself was already producing weekly reports (visible in `/dashboard/manager`); 3Y just wraps a WhatsApp delivery around the existing pipeline. **Sprint 3Y is code-complete; first real delivery will happen automatically when the Meta token is refreshed.**

---

### 10.43 Sprint 3W — Internal Hygiene (DONE, commit `451beae`) — 2026-05-15

Three small, unrelated quality fixes batched into one commit because they're all "<10 minutes each" cleanup work that was building up:

**1. Cleanup agent_runs noise eliminated.** The cleanup cron at `src/app/api/cron/cleanup/route.ts` was appending an `agent_runs` row at the end of each run with `tenant_id: null`. The `agent_runs.tenant_id` column is `NOT NULL`, so every cleanup invocation produced a warning log: `[cleanup] Could not log to agent_runs (likely tenant_id NOT NULL constraint or RLS): null value in column "tenant_id" of relation "agent_runs" violates not-null constraint`. The insert was always wrapped in try/catch so cleanup still functioned, but the daily warning was confusing — first-time readers thought cleanup was broken. **Fix:** removed the insert entirely. Cleanup is platform-scope (not tenant-scope) and has no meaningful tenant_id; results live in the JSON response body and per-task `console.log` lines, both visible in Vercel function logs. If durable cleanup telemetry is needed later, the right move is a dedicated `system_runs` table — not relaxing `agent_runs` constraints. The `runId` (`randomUUID()`) is retained and prefixed to each console.log for cross-task correlation.

**2. Engine repo README rewrite.** The `README.md` at the repo root was still the Next.js scaffold boilerplate ("This is a Next.js project bootstrapped with `create-next-app`..."). Anyone opening the repo — a future engineer, a potential design partner CTO, a lawyer doing due diligence — would see auto-generated content that says nothing about Spike. Replaced with a proper README: status (pre-launch, external blockers only), architecture table (frontend/backend/db/AI/messaging/auth/queue/hosting), agents table (10 agents with their triggers/outputs/recipients), project structure tree with one-line descriptions per top-level folder, local-dev setup with env-var documentation, pre-push gates (tsc + build), deployment notes, security section (RLS, server-side auth, webhooks HMAC, cron secrets, PII scrubbing, prompt injection, defamation guard, GDPR data residency), and a pointer to `CLAUDE.md` as the operational source of truth.

**3. Watcher route comment cleanup.** The Sprint 3X version of `src/app/api/cron/watcher/route.ts` inherited a comment block that claimed the cron schedule was `"0 * * * *"` (hourly), but `vercel.json` has always had `0 6 * * *` (daily). Pre-existing documentation drift, not introduced by 3X — but easy to fix while in the same area. Four comment edits (zero logic changes): `"Hourly cron"` → `"Daily cron"`, the schedule string → `"0 6 * * *"` with UTC/IL timezone note, `"within an hour"` → `"within a day"` (about the cron-catches-missed-webhook semantics), and `"across hourly runs"` → `"across daily runs"` (about the dedupe behavior). Confirmed via `grep -i hourly` afterward — zero remaining occurrences in the file.

**Why one commit:** all three are pure hygiene with no logic interaction; bundling them keeps the commit history concise and signals "today's cleanup pass" rather than spreading three trivial commits across unrelated PRs.
...
**Resolution (2026-05-13, commit `c4b6942`):** fixed via the **type extraction pattern** — all types and constants moved out of `src/app/dashboard/settings/actions.ts` into a neutral sibling `src/app/dashboard/settings/types.ts`. The "use server" file now contains ONLY `"use server"` + imports + the async exported function + a private `validate()` helper. Zero type exports, zero constant exports. The settings page (`page.tsx`) imports types from `./types`, the form (`settings-form.tsx`) imports types from `./types` and the function from `./actions`. Verified end-to-end on production: clicking "שמור הגדרות" produces the success toast "ההגדרות נשמרו", brief persists to `tenants.config.business_brief`, zero Vercel runtime errors over a 30-min observation window. The general principle is documented in §15.33 (type extraction pattern for Turbopack `"use server"` quirks). **Note:** the §15.27 "tsc + npm run build before every push" rule was instrumental — the fix required clean `.next` cache (§15.5) and `npm run build` gate before commit, both of which earlier attempts had skipped.

Historical record of the bug (preserved for archaeology):
...
**Worked on Edge, broke on Node:** initially this was believed to be the cause, but the resolution (2026-05-13) found that the settings page had `runtime = "edge"` set throughout AND still crashed. The real cause was Turbopack/SWC mishandling certain type usage patterns inside `"use server"` files **regardless of runtime**. Edge vs Node was a red herring.
...
4. Define `BusinessOwnerGender` LOCALLY in `actions.ts` as `"male" | "female" | "plural"` literal union, removing all imports from `gender-lock.ts` → still crashed (suggesting the bug is more subtle than just the import path) — likely stale Vercel cache contributed; subsequent attempts didn't run `npm run build` locally before push (§15.27 violation), so the same broken bundle persisted
5. Add comprehensive try-catch + logging → didn't address root cause; the crash is BEFORE the function runs

**Why the type extraction pattern worked (where attempts 1-5 didn't):**
The bug is triggered when Turbopack's SSR chunk for a `"use server"` file references a TYPE in a context that creates a runtime symbol reference (interface fields, `satisfies` clauses, etc). Even local type definitions can trigger this under the wrong circumstances. The solution removes the entire surface: the `"use server"` file has NO type references at all. Types live in `types.ts` (a neutral file with no directives, no `"use server"`, no `"server-only"`), which Turbopack handles correctly. This decoupling sidesteps the bug rather than fighting it.

**Earlier "Recommended approach" (now superseded by the type extraction fix):** rollback to `f19c0fe`, build Sprint 3I from scratch on Edge runtime. This was the plan as of 2026-05-13 morning. The actual fix turned out simpler — type extraction worked on Node runtime without rolling back. The 5 prior attempt commits were preserved and the fix built on top of them.
...
### 15.33 Type Extraction Pattern for Turbopack `"use server"` Quirks (Sprint 3I resolution lesson) ⚠️

**The rule:** a `"use server"` file should contain ONLY the `"use server"` directive, imports, the async exported function(s), and pure helper functions. **No type exports. No constant exports. No interfaces beyond the function signature's inline parameter type if absolutely needed.** All types and constants belong in a sibling `types.ts` (or `constants.ts`) file with NO directives — Turbopack handles those perfectly, and consumers can import from there.

**Why this matters:** Turbopack/SWC's handling of `"use server"` files is brittle around type usage (see §15.29 for the full history). Even type-only imports can produce runtime `ReferenceError` crashes under the wrong circumstances. The bug surface is essentially unpredictable — you can't tell from reading the code which patterns will trip it. The defensive strategy is to keep `"use server"` files type-free.

**Concrete pattern (Spike's settings folder, the test case for §15.29 resolution):**
```
src/app/dashboard/settings/
├── page.tsx           ← Server Component, imports types from ./types, function from ./actions
├── actions.ts         ← "use server"; imports types from ./types; defines async updateTenantSettings + private validate(); NO type/constant exports
├── types.ts           ← Neutral file; defines TenantSettingsInput, UpdateTenantSettingsResult, BUSINESS_BRIEF_MAX_LENGTH, VALID_VERTICALS, VALID_GENDERS, re-exports BusinessOwnerGender
└── settings-form.tsx  ← "use client"; imports function from ./actions; imports types from ./types
```

The split lets all three consumers (page, action, form) share types via `./types` while the action file stays minimal and Turbopack-safe.

**Anti-pattern (the §15.29 trap):**
```typescript
// actions.ts — BAD
"use server";
import type { BusinessOwnerGender } from "@/lib/safety/gender-lock";
export type { BusinessOwnerGender };  // ← runtime reference under the hood
const VALID_GENDERS = ["male", "female"] as const satisfies readonly BusinessOwnerGender[];  // ← runtime reference
export async function updateTenantSettings(...) { ... }
```

This crashes at module evaluation with `ReferenceError: BusinessOwnerGender is not defined` even when the type is logically erasable. The fix is structural, not patchable — move types out, period.

**Verification:** if you suspect a `"use server"` file has this issue, search it: `grep -E "(interface |type [A-Z]|satisfies|: [A-Z]\w+)" actions.ts`. Any hit is a candidate to relocate. The action file's TypeScript surface should be: `"use server"` + a few imports + one async function signature + body. Nothing more.

**Sibling rules:**
- §15.5 — clean `.next` cache before reproducing or fixing (stale cache hid the §15.29 fix multiple times)
- §15.27 — run `npm run build` locally (not just `tsc --noEmit`) before every push; Turbopack issues only manifest in build, not in type-check
- §15.26 — `"use server"` cannot export non-async values (a stricter sibling of this rule for constants specifically)
...
**Sample first reply:**
> קראתי את CLAUDE.md. Spike Engine — 9 סוכני AI מול לקוח (Morning, Watcher, Reviews, Hot Leads, Social, Manager, Sales, Inventory, Growth) + cleanup פנימי, drafts-only **למעט carve-outs ל-owner-self loopback** (3M Morning daily, 3X Watcher critical/high, 3Y Manager weekly — ראה §15.25 + §10.39 + §10.41 + §10.42), עברית RTL, Anthropic only. Stage 1 הושלם במלואו + Post-Stage-1 polish דרך 1.16 + Sprint 2 Batch 2C/2D + 3A + 2 RLS migrations (025 memberships recursion, 026 events tenant SELECT) + Dashboard runtime fix (`7539dcd`) שמתקן 4 סוכנים כבדים מ-25s Edge → 60s nodejs. **שלוש WhatsApp deliveries אמיתיות הוכחו end-to-end:** Growth Reactivation (דנה כהן) ב-2026-05-08, Sales quick_response (מוחמד אבו ראס) ב-2026-05-09, Morning daily_summary auto-send לבעל-העסק (+972509918196) ב-2026-05-10. /dashboard/approvals מרנדר messageHebrew נכון; double-execute race ב-drafts.ts מוקשח (§15.23 mitigations 1+2); helpers משותפים ב-`src/lib/whatsapp/helpers.ts`. **All 5 LLM call sites optimally cached** (Manager + Inventory direct, Sales×2 + Social via `withGenderLock` — §15.32). **Cron:** **9 jobs** ב-vercel.json (Morning `0 4 * * *`, Watcher `0 6 * * *`, Manager `0 5 * * 0`). הכל בייצור על app.spikeai.co.il. **Sprint 3I — Business Context Brief — COMPLETE ✅ (2026-05-13):** Phase 1 unblocked דרך §15.33 type extraction pattern (`c4b6942`). Phase 2: Batch 1 (`f39b26b` settings UI) → Batch 2 (`806904a` 4 Sonnet agents Reviews/Sales×2/Social) → Batch 3 (`d1eba6a` Growth). Batch 4 descoped (Morning/Watcher/Manager/Inventory owner-facing; Hot Leads classification-only). **5 customer-facing agents מזריקים brief.** אומת end-to-end: brief="סלון יופי...אוהבת לקרוא ללקוחות 'יקירה'" → Social יצר 3 פוסטים עם "יקירים ויקירות" ו"טיפול קרטין"; Growth יצר טיוטה ל-דנה כהן עם "דנה יקירה!" + "חידוש הקרטין" + "הקרטין מחזיק בערך עד כאן בדיוק" + "סליחה על זה". **Owner-facing trinity — COMPLETE ✅ (2026-05-15):** Morning daily 3M ✅ + Watcher real-time 3X ✅ (`e3bff86` — auto-send critical/high alerts, severity threshold `severityRank<=high`, diff-based dedupe על `${category}|${source}|${occurredAt}` — אפס DB column חדש) + Manager weekly 3Y ✅ (`3decc46` — Sunday 08:00 IL, compact teaser render עם empty-section omission, weekly idempotency דרך `weekStart=Sunday 00:00 UTC`). End-to-end WhatsApp delivery של 3X+3Y deferred עד refresh של Meta access-token (post-עוסק-מורשה); logic מאומת ע"י SQL inspection של historical Watcher runs. **Sprint 3W — internal hygiene ✅ (2026-05-15, `451beae`):** הסרת `agent_runs` insert מ-cleanup (סוף לdaily warning log), README חדש למנוע (החליף Next.js boilerplate), 4 comment fixes ב-watcher route (hourly→daily matching vercel.json). **לקחים חדשים נוספו:** §15.26 (`"use server"` לא יכול לייצא non-async), §15.27 (`tsc --noEmit` ≠ `next build`), §15.28 (Vercel Hobby: Edge 25s vs Node 60s), §15.29 (Turbopack import type bug — RESOLVED), §15.30 (`agent_runs` schema + cleanup SQL), §15.31 (Compress-Archive collisions), §15.32 (`withGenderLock` caching), §15.33 (type extraction pattern for "use server"), §10.40 (3I Phase 2 docs), §10.41 (3X docs), §10.42 (3Y docs), §10.43 (3W docs). **Latest commits:** `451beae` (3W hygiene), `3decc46` (3Y Manager), `e3bff86` (3X Watcher), `af59fe2` (Batch 5b backfill), `d1eba6a` (3I Batch 3 Growth). חוסמים חיצוניים: עוסק מורשה / Meta Business verification / מספר טלפון עסקי. אופציונלי-לא-חוסם: Vault encryption ל-access_token, sonner Toaster migration, Suspense pattern לדפים נוספים, marketing landing alignment, 3C voice notes, 3D smart waitlist, 3E GreenInvoice, 3F Google Calendar, 3G AI-driven brief auto-extractor (depends on 3I — foundation now stable), 3H self-service WhatsApp connection, Inngest fire-and-forget לסוכנים כבדים. **החלטות אסטרטגיות נעולות (§19):** pricing **OPEN QUESTION** — §19.1 thesis "single package, all agents" עומדת אבל המספר המדויק TBD; BSP=360dialog; wedge=[אשר] button + voice notes + no-shows ROI; channel=periphery + bookkeepers + Achiya. מה אתה רוצה לעשות?
...
| `3decc46` | feat(manager): weekly Sunday WhatsApp auto-send to owner via WhatsApp + vercel.json cron entry (`0 5 * * 0` = Sunday 08:00 IL) — Sprint 3Y, see §10.42 |
| `e3bff86` | feat(watcher): auto-send critical/high alerts to owner via WhatsApp with diff-based dedupe — Sprint 3X, see §10.41 |
| `af59fe2` | docs(claude): backfill Batch 1/2/3 hashes + mark Sprint 3I COMPLETE + Growth proof quote (Batch 5b) |
| `d1eba6a` | feat(agents): business_brief injection in Growth Sonnet draft path (Sprint 3I Phase 2 Batch 3 — see §10.40 + §15.32 cache discussion) |
| `d5ca7ad` | docs(claude): sweep for Sprint 3I Phase 2 + §15.29 resolution + §15.33 type extraction lesson + §10.40 phase 2 doc + §6.1 9 agents + §13 360dialog removal (Batch 5) |
| `806904a` | feat(agents): business_brief injection in Reviews/Sales/Social via withGenderLock (Sprint 3I Phase 2 Batch 2 — see §10.40) |
| `f39b26b` | feat(settings): business_brief textarea + save + load + plural-gender fix (Sprint 3I Phase 2 Batch 1 — see §10.40) |
| `c4b6942` | fix(settings): extract types out of "use server" actions.ts into neutral types.ts — resolves §15.29 ReferenceError + unblocks Sprint 3I Phase 2 (see §15.33 type extraction pattern) |
| `7539dcd` | fix(dashboard): switch from edge to nodejs runtime to give heavy Sonnet agents 60s instead of 25s — also re-applied Sprint 3I attempt (settings save fixed later by `c4b6942`) |
...
- **Sprint 3I — Business Context Brief (self-service brand voice)** ✅ **COMPLETE 2026-05-13** (Phase 1 unblock + Phase 2 Batches 1+2+3 shipped + Batch 4 descoped — see §10.40 for full per-batch documentation) — `/dashboard/settings` page where the owner writes a free-form Hebrew description of their business: what they sell, how they work, how they talk to customers, their style, anything that defines their voice. Stored in `tenants.config->>'business_brief'` (TOP-level JSONB key, max 2000 chars). Injected into the system prompt of **5 customer-facing agents** (Reviews ✅, Sales Followup ✅, Sales QR ✅, Social ✅, Growth ✅) as a `<business_voice>…</business_voice>` block placed AFTER the §15.32 cache breakpoint to preserve prompt caching. **Batch 4 (Morning + Hot Leads + Watcher + Manager + Inventory) DESCOPED** after analysis — Morning/Watcher/Manager/Inventory are owner-facing (§15.25), Hot Leads produces classification JSON only (downstream Sales QR already covered). Original "7 agents" claim was wrong; correct count is 5. **Effect on day-one production:** drafts already match the owner's voice on first generation — no manual editing required. **Validated end-to-end on demo tenant 2026-05-13** with brief = *"סלון יופי קטן בעין השופט. אני מתמחה בקרטין. אוהבת לקרוא ללקוחות 'יקירה'"*: Social produced 3 salon-themed posts with *"יקירים ויקירות"* + *"טיפול קרטין"*; Growth produced a reactivation draft for דנה כהן with *"דנה יקירה"* + *"חידוש הקרטין"* + *"הקרטין מחזיק בערך עד כאן בדיוק"* + natural Hebrew apology — all three brief-injection signals visible in one draft. **Resolution history:** Phase 1 was attempted across commits `408b4ed` → `cadde7c` → `7580b4d`/`1aa4877` (reverts) → `331ebb7` → `59feb7b` → `7539dcd` (5 attempts failed with `ReferenceError: BusinessOwnerGender is not defined at module evaluation` — see §15.29). The §15.33 type extraction pattern (commit `c4b6942`) resolved it by moving all types/constants out of the `"use server"` actions.ts into a neutral sibling `types.ts`. Phase 2 then shipped cleanly: `f39b26b` (Batch 1) → `806904a` (Batch 2) → `d1eba6a` (Batch 3). **Relationship to Sprint 3G:** 3G is the AI-driven version — auto-extract the brief from the business's website / Google reviews / Instagram during onboarding, pre-filling 3I's settings textarea. 3G now has a stable foundation to build on.
- **Sprint 3X — Watcher auto-send alerts to owner** ✅ **DONE 2026-05-15** — second Iron-Rule carve-out (§15.25) applied to Watcher's critical/high-severity alerts. Same `lookupWhatsAppIntegration` + `wasContactedInLast24h` + `mapSendErrorToHebrew` + `sendWhatsAppMessage` helpers from §10.39 — zero new shared code. Threshold: `severityRank(severity) <= severityRank("high")` (critical + high tiers; medium/low remain dashboard-only). Dedupe: diff-based against previous successful Watcher run's `output.alerts` via `${category}|${source}|${occurredAt}` signature — no new DB column. Single file modified (`src/app/api/cron/watcher/route.ts`); refactored from sequential loop into chunked `Promise.allSettled(5)` matching Morning's pattern. See §10.41. Commit `e3bff86`. End-to-end WhatsApp delivery deferred along with 3Y on Meta access-token refresh; SQL inspection confirmed the dedupe + filter logic would have triggered correctly on historical runs (1 critical/high on 05-14 06:18 → would send 1; 2 critical/high on 05-13 15:43 → would send 2; etc.).
- **Sprint 3Y — Manager weekly auto-send to owner** ✅ **DONE 2026-05-15** — third Iron-Rule carve-out completes the owner-facing trinity (Morning daily 3M + Watcher real-time 3X + Manager weekly 3Y). NEW route `src/app/api/cron/manager/route.ts` + 9th cron entry in `vercel.json` (`0 5 * * 0` = Sunday 05:00 UTC = 08:00 IL Sunday morning, one hour after Morning). Idempotency: per-tenant weekly check via `started_at >= weekStart` where `weekStart = Sunday 00:00 UTC` of current calendar week — protects against retry storms and manual-trigger overlap. Manager output (5 rich sections per `MANAGER_AGENT_OUTPUT_SCHEMA`) rendered as a compact 5-10 line teaser via `renderManagerSummary` — empty sections omitted, click-through to `/dashboard/reports` for the full report. See §10.42. Commit `3decc46`.
