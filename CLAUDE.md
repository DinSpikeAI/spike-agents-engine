...
>
> **Last updated:** 2026-05-13 evening (post-session: **Sprint 3I COMPLETE** + §15.29 RESOLVED + 4 batches shipped + Batch 4 DESCOPED). **The killer-differentiator is live in production:** every customer-facing draft now matches the owner's voice on first generation. **What shipped today:** (1) §15.29 type-extraction fix `c4b6942` — moved all types/constants out of the `"use server"` actions file (`src/app/dashboard/settings/actions.ts`) into a neutral sibling `types.ts`, resolving the 5-attempt `ReferenceError: BusinessOwnerGender is not defined at module evaluation` Turbopack bug; (2) Sprint 3I Phase 2 Batch 1 `f39b26b` — settings UI for `business_brief` (Card 3 textarea with 2000-char limit, save action, page loader, plural-gender fix as bonus); (3) Phase 2 Batch 2 `806904a` — brief injection into 4 Sonnet agents (Reviews, Sales Followup, Sales QR, Social) via `withGenderLock` extended with optional 3rd `brief` param; (4) Phase 2 Batch 3 `d1eba6a` — brief injection into Growth Sonnet draft path via a 3rd NON-cached system block placed after the two existing cached blocks (preserves §15.32 caching when only brief changes); (5) Batch 5 docs sweep `d5ca7ad` + Batch 5b backfill (this commit). **Sprint 3I scope correction:** the original §19.7 claim of "7 agents to inject" was wrong. **5 customer-facing agents** receive brief (Reviews + Sales×2 + Social via Batch 2; Growth via Batch 3). Morning, Watcher, Inventory, Manager are owner-facing — no customer voice to imitate. Hot Leads produces classification JSON only — no customer-facing text (downstream Sales QR is the text generator and already has brief via Batch 2). Batch 4 formally descoped these 5 agents. **Validated end-to-end on demo tenant 2026-05-13 with brief** = *"סלון יופי קטן בעין השופט. אני מתמחה בקרטין. אוהבת לקרוא ללקוחות 'יקירה'"*, vertical = "יופי / מספרה": **Social** produced 3 salon-themed posts including evening post with *"שתפו בתגובות, יקירים ויקירות"* and afternoon post with *"טיפול קרטין, מסכות משקמות"*. **Growth** produced a reactivation draft for דנה כהן: *"היי דנה יקירה! ראיתי שפנית בנוגע לחידוש הקרטין ולא חזרתי אלייך, סליחה על זה. הקרטין מחזיק בערך עד כאן בדיוק, אז זה הזמן המושלם לתאם 😊 מתי נוח לך?"* — note "דנה יקירה" (brief's "יקירה" address combined with the customer's actual name) + "חידוש הקרטין" + "הקרטין מחזיק בערך עד כאן בדיוק" (specialty knowledge from brief manifesting as professional voice) + "סליחה על זה" (natural Hebrew apology, not AI-clichéd). **2 new lessons added (§15.33 type extraction pattern, §10.40 Sprint 3I Phase 2 documentation).** **Earlier in this period (Sprint 3M, 2026-05-10):** daily auto-send of the Morning agent's Hebrew summary to the **business owner's** WhatsApp at 07:00 IL — Spike's third real WhatsApp delivery (owner-self loopback per §15.25). **Earlier same day 2026-05-13:** dashboard runtime fix `7539dcd` (edge → nodejs, gave heavy Sonnet agents 60s budget instead of 25s) + `agent_runs` cleanup of 12 stuck `running` rows. **The product is functionally complete for design partner #1.** External blockers only: עוסק מורשה / Meta Business verification / business phone number (paperwork, not code). **Strategic decisions locked — see §19** (pricing is OPEN QUESTION — §19.1 thesis "single package, all agents" stands but exact number TBD; §12.1's old Solo/Pro/Chain table is deprecated, see header note in §19.1): BSP 360dialog primary, Meta Cloud direct fallback; wedges = [אשר] button (TM-pending) → voice notes → no-shows ROI; channel = periphery cities + bookkeepers + Achiya rev-share. **Latest commits (newest first):** `d1eba6a` (Batch 3 — Growth), `d5ca7ad` (Batch 5 docs sweep), `806904a` (Batch 2 — 4 Sonnet agents), `f39b26b` (Batch 1 — settings UI), `c4b6942` (§15.29 type extraction fix).
...
### 10.40 Sprint 3I Phase 2 — Business Brief Injection into Customer-Facing Agents (Batches 1+2 DONE, Batch 3 PENDING, Batch 4 DESCOPED) — 2026-05-13
...
**Sample first reply:**
> קראתי את CLAUDE.md. Spike Engine — 9 סוכני AI מול לקוח (Morning, Watcher, Reviews, Hot Leads, Social, Manager, Sales, Inventory, Growth) + cleanup פנימי, drafts-only **למעט carve-out של 3M ל-owner-self loopback** (ראה §15.25 + §10.39), עברית RTL, Anthropic only. Stage 1 הושלם במלואו + Post-Stage-1 polish דרך 1.16 + Sprint 2 Batch 2C/2D + 3A + **3M (Morning auto-send + helpers extraction = 3B absorbed)** + 2 RLS migrations (025 memberships recursion, 026 events tenant SELECT) + **Dashboard runtime fix (`7539dcd`, 2026-05-13)** שמתקן 4 סוכנים כבדים שנכשלו ב-25s Edge timeout — עכשיו רצים על nodejs runtime עם 60s. **שלוש WhatsApp deliveries אמיתיות הוכחו end-to-end:** Growth Reactivation (דנה כהן) ב-2026-05-08, Sales quick_response (מוחמד אבו ראס) ב-2026-05-09, Morning daily_summary auto-send לבעל-העסק (+972509918196) ב-2026-05-10. /dashboard/approvals מרנדר messageHebrew נכון לאחר 3A; double-execute race בdrafts.ts מוקשח (§15.23 mitigations 1+2); helpers משותפים ב-`src/lib/whatsapp/helpers.ts` (3M). **All 5 LLM call sites already optimally cached** (Manager + Inventory direct, Sales×2 + Social via `withGenderLock` — see §15.32). **Cron:** 8 jobs ב-vercel.json, Morning ב-`0 4 * * *`. הכל בייצור על app.spikeai.co.il. **Sprint 3I — Business Context Brief — COMPLETE ✅ (2026-05-13):** Phase 1 unblocked דרך §15.33 type extraction pattern (`c4b6942` — הוצאת כל הtypes/constants מ-"use server" actions.ts לקובץ `types.ts` נייטרלי). Phase 2 Batch 1 (`f39b26b` — settings UI: Card 3 textarea 2000 chars, save action, page loader, plural-gender bonus fix) → Batch 2 (`806904a` — הזרקה ל-4 Sonnet agents Reviews/Sales×2/Social דרך withGenderLock עם פרמטר brief אופציונלי 3rd) → Batch 3 (`d1eba6a` — Growth Sonnet draft path, brief כ-block 3 ללא cache_control). Batch 4 descoped (Morning + Watcher + Manager + Inventory owner-facing; Hot Leads classification-only). **5 סוכנים customer-facing מזריקים brief בייצור.** אומת end-to-end על demo tenant: brief="סלון יופי...אוהבת לקרוא ללקוחות 'יקירה'" → Social יצר 3 פוסטים עם "יקירים ויקירות" ו"טיפול קרטין"; Growth יצר טיוטה ל-דנה כהן עם "דנה יקירה!" + "חידוש הקרטין" + "הקרטין מחזיק בערך עד כאן בדיוק" + "סליחה על זה" — natural Hebrew, אישי, מקצועי. **לקחים חדשים נוספו:** §15.26 (`"use server"` לא יכול לייצא non-async), §15.27 (`tsc --noEmit` ≠ `next build`), §15.28 (Vercel Hobby: Edge 25s vs Node 60s), **§15.29 (Turbopack import type bug — RESOLVED 2026-05-13)**, §15.30 (`agent_runs` schema + cleanup SQL), §15.31 (Compress-Archive collisions), §15.32 (`withGenderLock` caching), **§15.33 (type extraction pattern for "use server" — Sprint 3I resolution)**, §10.40 (Sprint 3I Phase 2 full per-batch documentation). **Latest commits:** `d1eba6a` (Batch 3 Growth), `d5ca7ad` (Batch 5 docs), `806904a` (Batch 2 4 agents), `f39b26b` (Batch 1 UI), `c4b6942` (§15.29 fix). חוסמים חיצוניים: עוסק מורשה / Meta Business verification / מספר טלפון עסקי. אופציונלי-לא-חוסם: Vault encryption ל-access_token, sonner Toaster migration (alert→toast), Suspense pattern לדפים נוספים, marketing landing alignment, 3X (Watcher auto-send), 3Y (Manager weekly auto-send), 3C voice notes, 3D smart waitlist, 3G AI-driven brief auto-extractor (depends on 3I — foundation now stable), Inngest fire-and-forget לסוכנים כבדים. **החלטות אסטרטגיות נעולות (§19):** pricing **OPEN QUESTION** — §19.1 thesis "single package, all agents" עומדת אבל המספר המדויק TBD; §12.1's old Solo/Pro/Chain table deprecated; BSP=360dialog; wedge=[אשר] button + voice notes + no-shows ROI; channel=periphery + bookkeepers + Achiya. מה אתה רוצה לעשות?
